    ...
    try {
        ...
        for (var x in channels) {
            ...
            if (channels[x] == tourschan || parameters.event) {
                ...
                sendChanHtmlAll("<timestamp/> " + (startsWithVowel(tier) ? "An " : "A ") + "<b><a href='http://wiki.pokemon-online.eu/page/Tiers:"+tier.replace(/ /g,"_")+"'>"+tier+"</a></b> "+(!tours.tour[key].event ? "tournament" : "event")+" has opened for signups in #"+sys.channel(tourschan)+"!", channels[x]);
                sendChanAll("Use /cjoin Tournaments and type /join to enter the tournament!"), channels[x]);
            }
            if (!parameters.event) {
                sendChanAll(border, channels[x]);
            }
